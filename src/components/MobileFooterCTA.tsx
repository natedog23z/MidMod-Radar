...
export const MobileFooterCTA = () => {
  ...
  const { house, loading } = useHouse();
  
  // We don't need loading states here since the parent components will handle that
  return <>
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E9E8E3] p-4 space-y-3">
        ...
        <div className="flex gap-3">
          <button 
            className="flex-1 border border-[#E9E8E3] bg-white hover:bg-gray-50 px-4 py-3 rounded-full flex items-center justify-center gap-2 transition-colors" 
          ...
          <button 
            className={`flex-1 border border-[#E9E8E3] bg-white hover:bg-gray-50 px-4 py-3 rounded-full flex items-center justify-center gap-2 transition-colors ${isFavorited ? 'text-[#FF6B4A]' : ''}`}
            onClick={() => setIsFavorited(!isFavorited)} 
            aria-label={isFavorited ? 'Remove from favorites' : 'Add to favorites'}
            disabled={loading || !house}
          >
            <Heart 
              className={`w-5 h-5 ${isFavorited ? 'fill-current text-[#FF6B4A]' : 'text-[#101010]'}`} 
              strokeWidth={1.5} 
            />
            <span className={isFavorited ? 'text-[#FF6B4A]' : 'text-[#101010]'}>Favorite</span>
          </button>
        </div>
      </div>
